...
class GitUpdateDateTests(unittest.TestCase):
    ...
    def _post(self, root: Path, directory: str = "post"):
    ...
    def test_manual_policy_preserves_authored_value(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            posts = [post]
            apply_update_policy(
                BlogContext.create(root), {"update_policy": "manual"}, posts
            )
            self.assertEqual(posts[0].update, make_calver(2026, 2, 1))
    ...
    def test_first_article_commit_has_no_update_date(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            posts = [post]
            apply_update_policy(BlogContext.create(root), {"update_policy": "git"}, posts)

            self.assertIsNone(posts[0].update)
    ...
    def test_latest_article_asset_commit_becomes_update_date(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(posts[0].update, make_calver(2026, 3, 4))
    ...
    def test_article_rename_keeps_earlier_history(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            posts = [renamed]
            apply_update_policy(
                BlogContext.create(root), {"update_policy": "git"}, posts
            )

            self.assertEqual(posts[0].update, make_calver(2026, 5, 6))
    ...
    def test_shallow_repository_falls_back_to_manual_value(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                ...
                apply_update_policy(
                    BlogContext.create(directory), {"update_policy": "git"}, posts
                )
            self.assertEqual(posts[0].update, make_calver(2026, 2, 1))
