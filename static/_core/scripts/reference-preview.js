...
export function initReferencePreview() {
  const slot = document.querySelector("[data-reference-preview-slot]");
  if (!slot) return;
  ...
  const updatePlacement = () => {
    // A mobile sidebar may be hidden. Keep the floating dialog outside it.
    (desktopQuery.matches ? slot : document.body).append(preview);
  };
  updatePlacement();
